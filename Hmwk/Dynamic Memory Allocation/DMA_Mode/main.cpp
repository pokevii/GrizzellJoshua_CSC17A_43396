...
int *mode(const int *array,int arySize){
    ...
    for(int i=0; i < arySize-1; i++){
        if(ary[i] == num){
            ...
            if(count > mCount){ //If the count of the current number is larger than the count of the mode...
                mCount = count; //...update the mode, so it takes the current count.

            } else if (count == mCount){    //If the count is equal to the mode's count, that means that this number is also a mode.
                nmodes++;       //...and increase the number of modes by 1.
                tempAry[j] = num;
                
                if(tempAry[j] > 100 || tempAry[j] < -100){ //here's a REALLY stupid fix, tempary[0] would always be a garbage number
                    tempAry[j] = 0;
                }
                
                j++;
            }
            count = 1;          //reset the count for the next loop.
            num = ary[i];       //set the number for the next loop.
        }
    }
    ...
    modeAry[0]=nmodes;  //modeAry[0] is the number of modes.
    modeAry[1]=mCount;  //modeAry[1] is the frequency of the number(s).
    //if(nmodes+2 != ){
    //    modeAry[0] = 0;
    //}
    for(int i=0; i < nmodes; i++){
        modeAry[i+2] = tempAry[i];
    }
